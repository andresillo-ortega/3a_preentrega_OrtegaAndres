...
def agregar_carro(request):
    if request.method == 'POST':
        ...
        form = CarroForm()
    return render(request, 'gestion/agregar_carro.html', {'form': form})
...
def agregar_renta(request):
    if request.method == 'POST':
        ...
        form = RentaForm()
    return render(request, 'gestion/agregar_renta.html', {'form': form})
...
def buscar(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'gestion/resultados_busqueda.html', {
                'clientes': clientes,
                'carros': carros,
                'rentas': rentas
            })
        ...
        form = BuscarForm()
    return render(request, 'gestion/buscar.html', {'form': form})
